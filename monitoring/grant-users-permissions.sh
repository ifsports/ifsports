...
# Este script concede as permissões de monitoramento necessárias
# para os usuários existentes de cada banco de dados.
...
echo "Iniciando a concessão de permissões para monitoramento..."
...
for i in "${!containers[@]}"; do
  ...
  # --- AJUSTE FINO NO COMANDO SQL ---
  # Adicionamos 'pg_catalog.' para sermos mais específicos na permissão.
  sql_command="GRANT pg_monitor TO $sql_user; GRANT EXECUTE ON FUNCTION pg_catalog.pg_postmaster_start_time() TO $sql_user; GRANT CONNECT ON DATABASE $database TO $sql_user;"
